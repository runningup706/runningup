...
| Gate 8 — race, social, season, operations | **PARTIAL** | Race resolution, continent course traits, champion race plans, strategy choice and world restoration are playable end to end. Social, season and live-ops remain schema groundwork. |
...
| Domain engine | `node --test "tests/*.test.mjs"` | **190 pass / 0 fail** |
...
| Database suite | `pg_prove backend/supabase/tests/pgtap/*.sql` | **Files=7, Tests=1140, Result: PASS** |
...
| Region nodes | 192 | 192 | DATA_PASS |
| Running courses | 2304 | 2304 | DATA_PASS |
| Main races | 72 | 72 | DATA_PASS |
| Challenge races | 24 | 24 | DATA_PASS |
| Race formats | 7 | 7 | DATA_PASS |
| Challenge formats | 6 | 6 | DATA_PASS |
| Playable runners | 12 | 12 | DATA_PASS |
| Character episodes | 36 | 36 | DATA_PASS |
| Race techniques | 48 | 48 | DATA_PASS |
| Gear sets | 72 | 72 | DATA_PASS |
| Standard rival crews | 24 | 24 | DATA_PASS |
| Elite rival crews | 12 | 12 | DATA_PASS |
| Continent champions | 12 | 12 | DATA_PASS |
| Open race events | 4 | 4 | DATA_PASS |
| Apex 1000 final race | 1 | 1 | DATA_PASS |
...
been built. Continents playable: **0/12**. Runners playable: **0/12**. See ADR-003.

## Direction lock status

| Lock | State | How it is held |
|---|---|---|
| DL-1 · 1000 km final, 121 checkpoints | **PASS** | Enum order, 4 CHECK constraints, 44 pgTAP tests, 18 unit tests, static scan |
...
| DL-6 · running race, not a combat game | **PASS** | Combat catalogue dropped by migration 0012 and asserted absent by pgTAP; `battle_node` and non-running gear axes throw; 13 scanner patterns, self-tested |
...
bash scripts/db.sh                      # 2. confirm the database suite is still green
...
|---|---|---|
| `docs/RLS_MATRIX.md` | `pg_catalog` on a migrated database | `scripts/db.sh` |
| `docs/DATABASE_SCHEMA.md` | 55 relations, 441 columns, 256 constraints | `scripts/db.sh` |
| `artifacts/test-evidence-manifest.json` | 7 suites that ran, 7 items blocked | `scripts/db.sh` |
