...
namespace RunningUp.UI
{
    ...
    public sealed class V11HudController : MonoBehaviour
    {
        ...
        {
            ShowScreen(2);
            ShowToast("Daily Run Contract · 검증 러닝 1회로 완료");
        ...
        private void RefreshRuntimeTexts()
        {
            ...
            {
                strideLeapText.text =
                    $"STRIDE LEAP\n충전 {progress.strideLeapChargeMeters}m";
            }

            if (growthText != null)
            {
                growthText.text =
                    $"검증 러닝 {progress.lifetimeVerifiedMeters / 1000f:0.0}km · " +
            ...
            if (pendingUploadText != null)
            {
                pendingUploadText.text =
                    $"대기 {runtime.PendingUploadCount} · {HumanizeBackendStatus()}";
            }

            if (homeVerifiedText != null)
            {
                homeVerifiedText.text =
                    $"서버 승인 러닝 {progress.lifetimeVerifiedMeters / 1000f:0.0}km";
            }
        }
        ...
        private string HumanizeBackendStatus() =>
            backendSyncStatus switch
            {
                ...
                "uploading" => "업로드 중",
                "synchronized" => "동기화 완료",
                // 서버가 이미 같은 러닝을 가지고 있었다. 사용자에게는 정상 완료다.
                "already_synchronized" => "동기화 완료",
                "network_retry" => "재시도 예약",
                "server_response_invalid" => "재시도 예약",
                "session_refresh_failed" => "로그인 갱신 필요",
                // 재시도 상한을 넘겨 큐에서 내렸다. 조용히 사라진 것처럼 보이면 안 된다.
                "upload_gave_up" => "업로드 실패 기록됨",
