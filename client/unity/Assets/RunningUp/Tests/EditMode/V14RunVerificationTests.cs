// V14 러닝 검증, 파일 수집, 성장 배분, 오프라인 중복 방지를 회귀 검증한다.
using System;
...
{
    public sealed class V14RunVerificationTests
    {
        [Test]
        ...
        public void OfflineQueueIsIdempotentAndRecoverable()
        {
            var path = Path.Combine(
                Path.GetTempPath(),
                $"runningup-v14-queue-{Guid.NewGuid():N}.json");
            try
            {
        ...
        public void OfflineQueueRecoversFromAtomicBackupAfterPrimaryCorruption()
        {
            var path = Path.Combine(
                Path.GetTempPath(),
                $"runningup-v14-queue-backup-{Guid.NewGuid():N}.json");
            try
            {
                var candidate = RunVerifier.VerifySummary(
                    RunSource.Gpx,
        ...
        public void HealthConnectDiscoveryPersistsPreviewUntilExplicitImport()
        {
            ...
            try
            {
                var bridge = objectUnderTest.AddComponent<V14AndroidRunBridge>();
        ...
        {
            const string progressKey = "runningup.v14.runner-progress";
            var queuePath = Path.Combine(
                Application.persistentDataPath,
                "v14-pending-runs.json");
            var previousQueue = File.Exists(queuePath)
            ...
            try
            {
                ...
                var runtime = objectUnderTest.AddComponent<V14RunRuntime>();
                var candidate = RunVerifier.VerifySummary(
                    RunSource.HealthConnect,
        ...
        public void SupabaseConfigAcceptsHostedHttpsAndExplicitLocalDevelopmentHosts()
        {
            ...
            try
            {
                var config = objectUnderTest.AddComponent<V14SupabaseRuntimeConfig>();
                config.Configure(
        ...
        public void SupabaseRpcPayloadAndResponsePreserveIdempotencyFields()
        {
            var candidate = RunVerifier.VerifySummary(
                ...
                5000,
                1800).Candidate;
            var json = V14SupabaseUploadContract.BuildJson(candidate);
            ...
            const string responseJson =
                ...
                "\"world_crown\":false}]";
            Assert.That(
                V14SupabaseUploadContract.TryParseResponse(
                    responseJson,
                    out var response),
                Is.True);
        ...
        {
            Assert.That(
                V14SupabaseUploadContract.Classify(
        ...
        public void SupabaseSessionTokenStaysInMemoryAndExpiresSafely()
        {
            ...
            try
            {
                var session = objectUnderTest.AddComponent<V14SupabaseSessionRuntime>();
                session.SetSession(
