// 기능 prototype과 Production 2.8등신 계약, animation, 세로 홈, 전투 잔재 제거를 검증한다.
using System;
...
{
    public sealed class V14RunnerQualityTests
    {
        private const string PrefabPath =
            "Assets/RunningUp/Generated/Prefabs/MyRunnerV14.prefab";
        ...
        [Test]
        public void FunctionalPrototypeRetainsItsExistingBudgetWithoutClaimingProductionArt()
        {
        ...
        [Test]
        public void ProductionPresentationContractMatchesLatestInGameDirection()
        {
            Assert.That(V14ProductionPresentationContract.Mode, Is.EqualTo("FIXED_CAMERA_2_5D"));
            Assert.That(V14ProductionPresentationContract.HeadRatioMinimum, Is.EqualTo(2.6f));
            Assert.That(V14ProductionPresentationContract.HeadRatioTarget, Is.EqualTo(2.8f));
            Assert.That(V14ProductionPresentationContract.HeadRatioMaximum, Is.EqualTo(2.9f));
            Assert.That(V14ProductionPresentationContract.ParallaxLayerMinimum, Is.EqualTo(3));
            Assert.That(V14ProductionPresentationContract.ParallaxLayerMaximum, Is.EqualTo(4));
            Assert.That(V14ProductionPresentationContract.VisiblePacerMinimum, Is.EqualTo(5));
            Assert.That(V14ProductionPresentationContract.VisiblePacerMaximum, Is.EqualTo(7));
            Assert.That(V14ProductionPresentationContract.Near3dPacerMaximum, Is.EqualTo(4));
            Assert.That(
        ...
        public void LiveJourneyHomeHasFiveTabsAndOneDailyContract()
        {
            ...
            var hud = UnityEngine.Object.FindFirstObjectByType<V14HudController>();
            ...
            Assert.That(
                UnityEngine.Object.FindFirstObjectByType<V14SupabaseRuntimeConfig>(),
                Is.Not.Null);
            Assert.That(
                UnityEngine.Object.FindFirstObjectByType<V14SupabaseSessionRuntime>(),
                Is.Not.Null);
            Assert.That(
                UnityEngine.Object.FindFirstObjectByType<V14VerifiedRunUploader>(),
                Is.Not.Null);
            ...
            for (var index = 0; index < names.Length; index++)
            {
                ...
                Assert.That(
                    Vector4.Distance(
                        ...
                        selected ? V14Palette.SkyBlue : Color.clear),
                    Is.LessThan(0.001f),
                    $"{names[index]} background");
                Assert.That(
                    Vector4.Distance(
                        button.GetComponentInChildren<Text>().color,
                        selected ? V14Palette.DeepNavy : V14Palette.MutedText),
                    Is.LessThan(0.001f),
                    $"{names[index]} label");
            }
        }
