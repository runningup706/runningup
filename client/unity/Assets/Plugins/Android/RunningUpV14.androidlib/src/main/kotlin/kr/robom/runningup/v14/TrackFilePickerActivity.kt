...
class TrackFilePickerActivity : Activity() {
    ...
    override fun onActivityResult(code: Int, result: Int, data: Intent?) {
        ...
        if (code != requestCode || result != RESULT_OK || data?.data == null) {
            RunningUpV14Bridge.send("OnTrackFileStatus", "cancelled")
            finish()
            return
        }
        ...
        try {
            ...
            if (extension !in setOf("gpx", "tcx", "fit")) {
                RunningUpV14Bridge.send("OnTrackFileStatus", "unsupported_extension")
                finish()
                return
            }
            val output = File(cacheDir, "v14-import-${UUID.randomUUID()}.$extension")
            contentResolver.openInputStream(uri).use { input ->
                requireNotNull(input) { "input stream unavailable" }
                output.outputStream().use { target -> input.copyTo(target) }
            }
            RunningUpV14Bridge.send("OnTrackFileSelected", output.absolutePath)
        } catch (error: Throwable) {
            RunningUpV14Bridge.send(
                "OnTrackFileStatus",
