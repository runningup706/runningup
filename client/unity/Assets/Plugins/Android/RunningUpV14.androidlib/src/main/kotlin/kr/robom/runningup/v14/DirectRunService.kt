...
package kr.robom.runningup.v14
...
class DirectRunService : Service(), LocationListener {
    companion object {
        const val ACTION_START = "kr.robom.runningup.v14.START_DIRECT_GPS"
        const val ACTION_PAUSE = "kr.robom.runningup.v14.PAUSE_DIRECT_GPS"
        const val ACTION_RESUME = "kr.robom.runningup.v14.RESUME_DIRECT_GPS"
        const val ACTION_STOP = "kr.robom.runningup.v14.STOP_DIRECT_GPS"
        const val ACTION_DISCARD = "kr.robom.runningup.v14.DISCARD_DIRECT_GPS"
        private const val notificationChannel = "runningup_run_capture"
        private const val notificationId = 711
        private const val preferenceFile = "runningup_v14_direct_run"
    ...
    override fun onLocationChanged(location: Location) {
        ...
        RunningUpV14Bridge.send("OnDirectGpsSample", payload.toString())
    }
    ...
    override fun onProviderDisabled(provider: String) {
        RunningUpV14Bridge.send("OnDirectGpsStatus", "provider_disabled")
    }
    ...
    private fun startCapture() {
        ...
        if (preferences.getBoolean(activeKey, false)) {
            ...
            if (paused) {
                RunningUpV14Bridge.send("OnDirectGpsStatus", "paused")
            } else {
                requestUpdates()
                RunningUpV14Bridge.send("OnDirectGpsStatus", "capturing")
            }
            return
        }
        ...
        if (
            ...
            RunningUpV14Bridge.send("OnDirectGpsStatus", "permission_required")
            stopSelf()
            return
        }
    ...
    private fun resumeCapture() {
        val preferences = getSharedPreferences(preferenceFile, MODE_PRIVATE)
        if (!preferences.getBoolean(activeKey, false)) {
            RunningUpV14Bridge.send("OnDirectGpsStatus", "no_active_session")
            return
        }
        restoreFromPreferences(preferences)
        if (
    ...
    private fun finishCapture() {
        ...
        RunningUpV14Bridge.send("OnDirectGpsFinished", payload)
    ...
    private fun discardCapture() {
        ...
        autoPaused = false
        slowSampleCount = 0
        RunningUpV14Bridge.send("OnDirectGpsStatus", "discarded")
        stopForeground(STOP_FOREGROUND_REMOVE)
        stopSelf()
    }
    ...
    private fun updateAutomaticPause(location: Location) {
        ...
        if (!preferences.getBoolean(autoPauseKey, true)) {
            if (autoPaused) {
                ...
                RunningUpV14Bridge.send("OnDirectGpsStatus", "auto_resumed")
            }
            slowSampleCount = 0
            return
        }
        ...
        if (!autoPaused && speed < 0.5f) {
            ...
            if (slowSampleCount >= 10) {
                ...
                RunningUpV14Bridge.send("OnDirectGpsStatus", "auto_paused")
            }
            return
        }
