...
| Check | Result |
|---|---|
| Single source of truth for locked numbers | All in `packages/domain/constants.mjs`, imported by factory, validator, simulators and tests; SQL mirrors them under CHECK constraints |
...
| Domain purity | `packages/domain/*` imports nothing but `constants.mjs` and node built-ins — no DB, no UI, no platform API |

## Findings

**F-08-1 (medium, accepted).** The reward formula exists twice: `packages/domain/reward.mjs` (client
